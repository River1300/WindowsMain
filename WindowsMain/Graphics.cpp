...
/*
알파 테스트
	BMP에 투명도를 적용할 때 사용한 기법이다. 단순히 그릴 이미지의 알파값이 특정 숫자 보다 적으면 해당 픽셀을 그리지 않는 방식
	따라서 디자이너가 채색한 알파 복성에 따라 외곽선이 남게 되며 확대/축소로 인해 이미지 손상이 발생하게 된다.

알파 블렌드
	이미 그려진 픽셀과 현재 필셀의 알파값을 혼합하여 연산을 수행하는 방식으로 투명도를 처리한다. 알파 테스트 보다는 나아보이지만,
	3D 게임은 회전하는 이미지를 보여줘야 한다. 그로 인해 알파 순서가 매우 중요하다.

	회전 각도에 따라 알파 연산을 수행하는 순서가 달라지면서 투명한 오브젝트가 사라지는 증상이 발생하게 된다. 정확하게는 투명한
	오브젝트 뒤에 있는 녀석들이 사라지는 증상이다.

	그래픽디자이너에게 요청해서 메시를 서로 분리하는 방법이 있다. 기본적으로 알파 연산은 오브젝트(메시) 단위로 일어나므로 분리해서
	그리면 해결된다. 물론 나무와 풀과 같은 것은 불가능
	이를 해결하기 위해 알파 정렬이라는 방식도 사용
		불투명한 오브젝트들만 먼저 그림
		반투명한 오브젝트는 뒤에서 부터 앞으로 그림
		그래도 문제가 생기면 알파테스팅 추가 적용

Alpha-To_Coverage
	위 문제를 해결하기 위해 등장한 개념, 위에서 알파테스팅을 섞는 방법을 설명했지만 Alpha Test는 퀄리티가 좋지 않음
	최신 그래픽카드들은 이러한 문제를 위해 Alpha-To-Coverage라는 기능을 지원
	혹은 Screen-Door 효과라고도 한다. 방충망 정도라고 보면 될 것 같다.

	최근 게임들을 보면 투명한 오브젝트들을 확대하면 점과 같은 것들이 보인다.
	이 기법이 바로 Alpha-To-Coverage 혹은 Screen-Door 효과이다.

	일반 알파 계산과는 달리 각각의 픽셀들을 뒤에 있는 녀석들과 블렌딩하는 방식이 아니라, 지정한 패턴으로 그릴것과 그리지 않는
	것을 지정하는 방식이다.
	다시말해, 텍스쳐의 알파 채널을 사용해 Anti-Aliasing의 마스크로 사용하는 방식으로 블렌딩 처리와는 다른 개념이다.
	이런 방식은 풀이나 나뭇잎 같은 복잡하고 투명한 영역이 많은 오브젝트가 모여 있는 장면을 표현하는데 주로 사용한다.
*/

/* 애니메이션 */

/*
애니메이션은 초당 수 프레임으로 이뤄진 이미지를 그려주면 된다. 여기서 초당 몇 프레임으로 표현되느냐가 애니메이션의 퀄리티를 결정한다.
따라서 하나의 애니메이션을 표현하려면 프레임 수에 따른 그림과 시간을 관리하는 기능이 필요하다.

프레임 기반 움직임 VS 시간 기반 움직임
int MainLoop()
{
	if(RIGHT ARROW)
	{
...
간단히 키 입력이 있으면 플레이어의 위치를 1씩 증가 시켰다. 그런데 이렇게 작성하면 문제가 발생한다.
낮은 사양 : 1초당 MainLoop 10번 실행 -> 1초에 x로 10만큼 이동
높은 사양 : 1초당 MainLoop 100번 실행 -> 1초에 x로 100만큼 이동
따라서, 프레임 기반으로 이동시키면 컴퓨터의 성능에 따라 서로 다른 결과가 나온다. 특히 서로 동기화가 필요한 멀피 플레이 게임에서는
절대 이런식으로 처리되어선 안된다.
이러한 문제를 막기 위해서는 시간 기반으로 이동해야 한다. 어느 컴퓨터이던 1초에는 x를 10만큼 이동시킬 필요가 있다.
그래서 대부분 게임 루프는 시간을 파라미터로 받는다.
int MainLoop(float deltaTime)
{
	if(RIGHT ARROW)
	{
		player.x += deltaTime * 10;
	}
}
deltaTime은 이전 mainloop 실행 후 경과된 시간을 의미한다. MainLoop가 0.01초 마다 불리면 deltaTime은 0.01초가 되고 x는 0.01씩 증가한다.
100번 호출되면 1초이며 그럼 0.01 * 10 * 100 = 10.0이 되서 1초에 10만큼 이동할 수 있게 된다.

고정 프레임 레이트 VS 가변 프레임 레이트
시간 기반으로 움직임을 처리해도 남은 문제가 있다. 바로 프레임레이트의 차이다. 위 예제에서는 프레임 레이트가 각각 초당 0.1회, 0.01회로
설명했지만 게임에서는 항상 고정된 프레임레이트가 되지 않는다.

갑자기 화면에 오브젝트가 많이 나타난다거나 하면 위와 같이 deltaTime이 요동칠 것이다. 이를 가변 프레임 레이트라고 한다.
가장 많이 볼 수 있는 게 흔히 말하는 탄막 슈팅 게임에서 총알이 엄청나게 쏟아져 나오면 느려진다. 이로 인해 보고 피할 시간이
늘어 난이도가 하락할 수 밖에 없다. 물론 탄막 슈팅은 이런 요소마저 게임성으로 이용하기도 한다.

이를 막기 위해서 고정 프레임 레이트를 사용한다. 주로 일본 게임들이 이 방식을 채택하는데, 일본의 게임 산업 출발은
대부분이 콘솔이고 성능이 획일화된 기기에서 실행된다. 따라서 어떤 콘솔로 게임을 해도 동일한 성능으로 실행될 수 있다.
그래서, 초당 프레임 수를 고정 시켜 버린다. 초당 30프레임으로 고정 시킨다고 가정하면 다음과 같이 코드를 작성하면 된다.
int MainLoop(float deltaTime)
{
	if(현재시간 - 이전 시간 < 0.03)
		break;
}
이번에는 현재 시간과 이전 시간의 차이를 따로 구해서 값이 0.03(초당 30프레임 = 1 / 30 = 0.0333)보다 작으면 게임을 실행하지 않는 것이다.
deltaTime이 그런 값인데 왜 delta를 사용하지 않을까?
가련 연속으로 deltaTime이 0.02인 상태로 10번 수행되면 if(delta < 0.03)으로 인해 무한정 스킵되어 버린다.
따라서 시간을 따로 구해서 판단해야 한다. 너무 빠른 프레임레이트는 건너뛰기 때문에 고정프레임레이트는 프레임 스ㅋ립이라고도 불렀다.

계산과 그리기의 분리
한 가지 더 신경 써 주어야 할 것은, 로직과 그리기의 분리다. 지금까지 작성한 코드나 위 코드에서 보면 MainLoop가 하나로 되어 있으면 여러가지 문제가 발생한다.

	안정된 게임 플레이 경험
		1초에 누가 더 많은 버튼을 연타하는 게임을 생각해 보면 1초에 60번 업데이트 되는 컴퓨터는 버튼을 60번 체크해서 눌렸는지
		확인하겠지만, 1초에 10번 업데이트 되는 컴퓨터는 10번 체크하게 된다. 당연히 불공편한 게임이된다. 그리기가 부담이 걸리더라도
		안정적인 로직 처리가 필요하므로 두 루틴을 구분해서 처리하는 것이 중요하다.
	물리연산
		최근 게임들에서는 현실적인 리액션을 위해 물리 엔진등을 적용하고 있다. 자연스러운 물리 처리를 위해서는 초당 최소한의
		연산이 보장되어야 한다. 위에서 살펴본 예제 코드처럼 MainLoop가 통채로 스킵되어 버리면 물리 연산 결과가 비정상적일 수 있다.
	플랫폼 이식
		요즘은 여러 프랫폼으로 게임을 출시한다. PC, 모바일, 콘솔 등 다양한 환경으로 개발하다 보면 DirectX, OpenGL, Metal등 다양한
		API를 사용하게 된다. 각 렌더링 API는 서로 다른 문법과 그래픽 이론을 사용하므로 이식이 필요하다. 정교하게 렌더링과 로직을 분리해
		두면 레더링 파트만 이식하면 된다.
*/