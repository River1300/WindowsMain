...
/*
방향과 크기 정보를 가지는 원소들의 집합
2차원 공간을 기준으로 설명하면 단순히 x 와 y 를 원소로 가지는 값
벡터만으로는 사실 큰 의미가 없다만 이러한 개념으로 파생되는 다양한 연산과 행렬들이 기하학 처리를 매우 쉽게 도와준다.

벡터는 방향과 크기를 가지고 있는데, 이 방향과 크기가 같다면 동일한 벡터가 된다.
두 벡터의 더하기 연산은 a 벡터만큼 이동한 후 b 벡터만큼 이동한 것과 같다.

-b는 b와 크기는 같지만 방향만 반대인 벡터이다. 이를 사용하여 a - b = a + (-b)로 볼 수 있다.
a, b와의 상관 관계를 나타내기 위해 결과를 다음과 같이 옮기면 a - b는 b에서 a로 향하는 벡터가 되는 것을 알 수 있다.
*/

/*
이 연산의 의미를 이해하기 위해서 간단한 예를 들자
레이싱 게임을 만들기 위해 다음과 같은 벡터 정보를 사용할 것이다.
	mPosition : 현재 위치를 저장하는 벡터
	mVelocity : 차량의 속도를 저장하는 벡터
	goalPosition : 골인 지점의 위치를 저장한 벡터
v 크기를 k배 해준다. 엑셀을 밟으면 현재 속도를 증가 시키면 된다. 증가되는 값을 k라고 하면, 현재 속도는 다음과 같이 계산할 수 있다.
	mVelocity = k * mVelocity;
화면에 골인지점까지의 방향을 안내하는 네비게이션을 만든다고 생각해 보면 항상 골인 지점을 향하는 화살표를 그리면 된다.
	goalPosition - mPosition;
따라서 각 연산은 다음과 같은 경우에 사용한다고 기억하면 좋다.
	스칼라 곱 : 벡터의 크기를 변경
	벡터 합 : v1으로 이동한 후 v2로 이동
	벡터 곱 : v2에서 v1으로 향하는 벡터
*/
...
/*
행과 열로 나열한 수를 의미
덧셈, 뺄셈은 매우 간단하지만 안타깝게도 잘 사용하지 않는다. 자주 사용되는 것은 행렬 곱셈
|a b| |e f| = |ae + bg af + bh|
|c d| |g h|   |ce + dg cf + dh|

ax + by = m
cs + dy = n
위 방정식을 풀기 위한 행렬이 다음과 같다.
|a b|*|x|=(m)=|ax + by|
|c d| |y| (n) |cx + by|
연립방정식의 해를 구하기 위한 행렬 표기법, 위 구조를 살펴보면 방정식이 아닌 행렬 x벡터라는 것도 알 수 있다.

ax + by =  m 형태는 우리가 지난 시간(그래픽카드 구현)에 사용한 직선의 방정식이다. 즉
연립방정식을 만족시키는 해라는 것은 두 직선에 모두 존재하는 점, 즉 교점을 의미하게 된다.

즉, 벡터에 어떠어떠한 연산을 수행한 결과 벡터를 알 수 있다는 의미가된다. 이러한 연산에 위치 변경,
크기 조절, 회전등이 있다.
이러한 것을 선형 대수학이라고 부른다. 공간에 존재하는 점은 모두 m x n행렬의 연산으로 이동/크기 조절/회전이 가능

하지만 화면의 도형에 대한 변환을 수행하려면 벡터만으로는 부족한 점이 있다.

벡터 공간에서는 v1과 v2는 크기와 방향이 같기 때문에 동일한 벡터이다. 하지만 기하학에서 점으로 표현할 때는 다른 값이어야만 한다.

아핀 공간
따라서 변환을 손쉽게 계산하기 위해서는 유클리드 기하학의 아핀 공간 개념을 빌려와야 한다.
어려운 개념이지만 간단히 정리하자면 점과 벡터를 구분지어서 생각하고 점과 벡터 간의 연산을 가능하게 해 주는 공간이라 생각하면 된다.

동차 좌표계
점과 벡터를 구분하기 위해 기존 차원에서 한 차원을 더해서 추가 정보를 기재하는 것, 2차 평면 공간에서 동차 좌표계는 다음과 같다.
	(x, y, 0) : (x, y)벡터를 의미한다.
	(x, y, 1) : (x, y)점을 의미한다.
*/
...
/*
2차원 좌표계에서 Translate, Scale, Rotation을 수행하기 위해서는 이러한 아핀 공간에서 수행되는 동차 좌표계가 필요
즉, 변환을 수행할 점은 (x, y, 1)형태가 될 거싱고 행렬은 3 * 3이 된다.
변환을 위해서는 행렬이 필요
아핀 공간에서는 위 행렬의 오른쪽 열은 항상 0, 0, 1로 고정되어 있다. 따라서 실제로 필요한 정보는 3 * 2 행렬만 있으면 된다.
*/

/* 도 Degree 와 라디안 Radian */

/*
일상 생활에 사용하는 각의 크기는 도 표현이다. 하지만 수학에서는 라디안 표기를 자주 사용한다.
국제 표준 역시 라디안 표기이며 도는 공식 단위는 아니지만 자주 사용하므로 인정하고 넘어가는 상태
	각도법
		평면 각도의 단위
		1회전을 36등분하여 정의
	호도법
		평면 각도를 중심으로하는 원의 호의 길이
		1회전 = 호의 길이 = 2파이

라디안 -> 도
	파이 : 180 = radian : degree
	degree = radian * 180/파이
도 -> 라디안
	파이 : 180 = radian : degree
